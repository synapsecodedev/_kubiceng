...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    // Fastify's inject handles JSON body automatically if passed as object, 
    // but sometimes needs string if content-type is manual.
    ...
    res.status(500).json({
      error: "Internal Server Error (api/index)",
